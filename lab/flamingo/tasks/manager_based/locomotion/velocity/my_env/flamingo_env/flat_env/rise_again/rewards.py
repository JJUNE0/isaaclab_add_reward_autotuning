...
def track_lin_vel_xy_link_exp_l1(
    ...
    lin_vel_error = torch.sum(
        torch.abs(env.command_manager.get_command(command_name)[:, :2] - asset.data.root_link_lin_vel_b[:, :2]),
        dim=1,
    )
...
def stop_stand_l2(
    ...
    asset: RigidObject = env.scene[asset_cfg.name]
    # compute the error
    wheel_vel_error = torch.sum(
        torch.square(asset.data.joint_vel[:, asset_cfg.joint_ids]),
        dim=1,
    )
    cmd = env.command_manager.get_command(command_name)
    
    return torch.where(torch.logical_and(cmd[:,0]==0,cmd[:,2]==0) ,wheel_vel_error, 0)


def track_lin_vel_xy_link_exp_l2(
    env: ManagerBasedRLEnv, std: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Reward tracking of linear velocity commands (xy axes) using exponential kernel."""
    # extract the used quantities (to enable type-hinting)
