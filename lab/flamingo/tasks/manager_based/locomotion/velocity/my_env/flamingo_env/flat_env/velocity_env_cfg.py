...
class CommandsCfg:
    ...
    gait_command = mdp.UniformGaitCommandCfg(
        resampling_time_range=(3.0, 7.0),  # Fixed resampling time of 5 seconds
        debug_vis=False,  # No debug visualization needed
        ranges=mdp.UniformGaitCommandCfg.Ranges(
            frequencies=(0.8, 1.6), # (1.5, 2.5),  # Gait frequency range [Hz]
            offsets=(0.0, 1.0),  # Phase offset range [0-1]
            durations=(0.3, 0.7),  # Contact duration range [0-1]
        ),
        initial_phase_time=2.0,  # Initial phase time to set phase offset to 0
    )
    ...
    base_velocity = mdp.UniformVelocityWithZCommandCfg(
        ...
        ranges=mdp.UniformVelocityWithZCommandCfg.Ranges(
            lin_vel_x=(-1.0, 1.0), lin_vel_y=(-0.0, 0.0), ang_vel_z=(-1.0, 1.0), pos_z=(0.1931942, 0.3531942)
        ),
