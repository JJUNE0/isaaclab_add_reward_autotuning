...
def error_joint_deviation_huber(
    ...
    """
    [수정됨] 기본 위치(0)에서 벗어난 조인트 각도 오차에 Huber loss를 적용합니다.
    - 기존의 torch.sum()을 제거하고 (N, num_joints) 벡터를 반환합니다.
    - |angle| < delta 이면 L2 (부드러움), |angle| >= delta 이면 L1 (폭발 방지)
    """
    # 1. 원시 오차 벡터 (목표=0)
...
# ---------------------------------------------------------
# 3) Base height → feature (N, 1)
#    - flat: target_height 고정 or command의 z(예: cmd[:,3]) 사용
#    - rough: RayCaster로 지면 z 평균을 더해 보정
# ---------------------------------------------------------
