...
class FlamingoCurriculumCfg(CurriculumCfg):
    modify_base_velocity_range = CurrTerm(
        ...
        params={
            ...
            "mod_range": {"lin_vel_x": (-0.1, 1.0), "ang_vel_z": (0.0, 0.0)},
            "num_steps": 25000,
        },
...
class FlamingoMOORewardsCfg:
    ...
    # 2. [신규] 갇혔을 때 바퀴 잠금 (Wheel Lock)
    # error_wheel_lock = MOORewardTermCfg(
    #     func=error_wheel_lock_by_integral,
    #     params={
    #         "wheel_cfg": SceneEntityCfg("robot", joint_names=[".*_wheel_joint"]),
    #         "command_name": "integral_position",
    #         "error_threshold": 0.2, # 20cm 이상 뒤쳐지면 발동
    #         "scale": 1.0,
    #         "kernel": "tanh"
    #     }
    # )
    ...
    #dof_acc_huber = MOORewardTermCfg(func= dof_acc_huber, params={"delta" : 1.0, "scale" : 1.0})
    joint_action_rate_huber = MOORewardTermCfg(
        func = joint_action_rate_huber, 
        params= {
            # "sensor_cfg": SceneEntityCfg("height_scanner"),
            #"command_name": "base_velocity",
            "kernel" : "tanh",
...
class FlamingoRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    def __post_init__(self):
       ...
       #! ************** scene & observations setup - 0 *********** !#
        ...
        # commands
        self.commands.base_velocity.ranges.lin_vel_x = (0.7, 0.8)
...
class FlamingoRoughEnvCfg_PLAY(FlamingoRoughEnvCfg):
    def __post_init__(self):
        ...
        # add base mass should be called here
        self.domain.randomize_mass.params["asset_cfg"].body_names = ["base_link"]
        self.domain.randomize_mass.params["mass_distribution_params"] = (-0.75, 1.0)
