...
class CommandsCfg:
    ...
    base_velocity = mdp.DiscreteAngularVelocityCommandCfg(
        ...
        ranges=mdp.DiscreteAngularVelocityCommandCfg.Ranges(
            lin_vel_x=(-1.0, 1.0), lin_vel_y=(-0.0, 0.0), ang_vel_z=(-2.0, 2.0), pos_z=(0.25, 0.45)
        ),
        initial_phase_time=5.0,
    )
...
class ObservationsCfg:
    ...
    class StackPolicyCfg(ObsGroup):
        ...
        velocity_commands = ObsTerm(
            func=mdp.generated_commands_height_normalized,
            params={"command_name": "base_velocity", "vel_scale": (2.0, 2.0, 0.25)},
            clip=(-100.0, 100.0),
        )
        joint_pos = ObsTerm(
    ...
    class NoneStackPolicyCfg(ObsGroup):
        ...
        velocity_commands = ObsTerm(
            func=mdp.generated_commands_height_normalized,
            params={"command_name": "base_velocity", "vel_scale": (2.0, 1.0, 0.25)}
        )
...
class EventCfg:
    ...
    physics_material = EventTerm(
        ...
        params={
            ...
            "dynamic_friction_range": (0.55, 0.8),
            "restitution_range": (0.0, 0.0),
            "num_buckets": 64,
        },
    )
    ...
    reset_base = EventTerm(
        ...
        mode="reset",
        params={
            "pose_range": {"x": (-0.5, 0.5), "y": (-0.5, 0.5), "z": (-0.05, 0.05), "yaw": (-3.14, 3.14)},
            "velocity_range": {
                "x": (-0.5, 0.5),
                "y": (-0.5, 0.5),
                "z": (-0.5, 0.5),
    ...
    reset_robot_joints = EventTerm(
        func=mdp.reset_joints_by_fold_offset,
        mode="reset",
        params={
            ...
            "interpolation_range": (0.0, 0.1),
            "position_noise_range": (-0.06, 0.06),
    ...
    # ------------------------------------------------------------------
    # Static (fixed-range, no curriculum) counterparts of DomainManagerCfg's
    # randomize_mass/randomize_com/randomize_gains/randomize_joints -- same
    # func, mode="reset" (re-sampled every episode), same max ranges. Disabled
    # (None) by default; a comparison run enables these AND disables the
    # matching DomainManagerCfg terms (see train.py --static_dr) to isolate
    # "plain EventManager DR from step 0" vs "DomainManager's gated curriculum"
    # as the only difference, rather than mixing both mechanisms.
    # ------------------------------------------------------------------
    static_randomize_mass: EventTerm | None = None
    static_randomize_com: EventTerm | None = None
    static_randomize_gains: EventTerm | None = None
    static_randomize_joints: EventTerm | None = None
...
class TitaActionsCfg:
    """Action specifications for TITA MDP."""
...
@configclass
class TitaObservationsCfg(ObservationsCfg):
    """Observation specifications for TITA MDP."""

    @configclass
    class StackPolicyCfg(ObsGroup):
        hip_shoulder_joint_pos = ObsTerm(
            func=mdp.joint_pos,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_leg_1", ".*_leg_2"])},
            noise=Unoise(n_min=-0.01, n_max=0.01),
        )
        leg_joint_pos = ObsTerm(
            func=mdp.joint_pos,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_leg_3"])},
            noise=Unoise(n_min=-0.01, n_max=0.01),
        )
    ...
    def __post_init__(self):
        super().__post_init__()
        if hasattr(self, "priv_extrio") and hasattr(self.priv_extrio, "is_contact"):
            self.priv_extrio.is_contact.params["sensor_cfg"].body_names = [".*_leg_4"]
        if hasattr(self, "priv_physical"):
            if hasattr(self.priv_physical, "actuator_kp"):
                self.priv_physical.actuator_kp.params["asset_cfg"].joint_names = [".*_leg_.*"]
            if hasattr(self.priv_physical, "actuator_kd"):
                self.priv_physical.actuator_kd.params["asset_cfg"].joint_names = [".*_leg_.*"]


@configclass
class TitaVelocityFlatEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the locomotion velocity-tracking environment."""
