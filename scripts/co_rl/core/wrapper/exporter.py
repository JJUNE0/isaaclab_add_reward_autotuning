...
def export_srm_as_onnx(srm, srm_fc, device, path, filename="srm.onnx", verbose=False):
    ...
    # Create the exporter instanceactor_critic
...
class ONNXWrapper(torch.nn.Module):
    ...
    def forward(self, history_buffer):
        return self.student.forward_onnx(history_buffer)
...
class EncoderONNXWrapper(torch.nn.Module):
    ...
    def __init__(self, student):
        super().__init__()
        self.student = student
        self.adaptation_module = student.adaptation_module
        
        # 차원 정보 가져오기
        self.single_obs_dim = student.single_obs_dim
...
def export_student_onnx(actor_critic: torch.nn.Module, output_path: str, device="cpu", verbose=False):
    ...
    print(f"[Export] Info - History Len: {history_len}, Frame Dim: {frame_dim}, Input Obs Dim: {num_obs}")

    # 더미 입력 생성
    batch_size = 1
    #dummy_obs = torch.randn(batch_size, num_obs, device=device)
    dummy_history = torch.zeros(batch_size, history_len, frame_dim, device=device)
    
    # [수정] Encoder Wrapper 사용
    wrapper = EncoderONNXWrapper(model)
    ...
    # 파일명 변경 (명확하게 encoder.onnx로 저장 권장)
    if output_path.endswith(".onnx"):
        # 사용자가 full path를 줬다면 그대로 사용하되, 파일명에 encoder가 없으면 경고하거나 그대로 둠
        pass
    else:
        output_path = os.path.join(output_path, "adaptation_encoder.onnx")
