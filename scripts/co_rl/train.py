...
# export PYTHONPATH=/home/cocel/Isaac/Isaac-RL-Two-wheel-Legged-Bot
...
parser.add_argument("--algo", type=str, default="sac", help="Name of the task.")
...
from core.runners import OnPolicyRunner, MOO_OnPolicyRunner, DistillationOnPolicyRunner
...
from lab.flamingo.isaaclab.isaaclab.envs import ManagerBasedMOORLEnvCfg
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg | ManagerBasedMOORLEnvCfg , agent_cfg: CoRlPolicyRunnerCfg):
    ...
    is_off_policy = False if agent_cfg.to_dict()["algorithm"]["class_name"] in ["PPO", "SRMPPO", "Distillation", "MOOPPO"] else True
    ...
    if is_off_policy:
        ...
        elif args_cli.algo == "mooppo":
            runner = MOO_OnPolicyRunner(env, agent_cfg.to_dict(), log_dir=log_dir, device=agent_cfg.device)
        elif args_cli.algo == "distillation":
            runner = DistillationOnPolicyRunner(env, agent_cfg.to_dict(), log_dir=log_dir, device=agent_cfg.device)
    ...
    # wandb.init(project=args_cli.task)
